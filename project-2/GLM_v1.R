...
#                             : and the rest in a group called regular duty                       
# Current AIC: 522.1747
# Current BIC: 552.8371
###################################################################
...
######The data will be sotred in a table named glmdata
...
# The code below groups the variable weight, from your table glmdata,
# into six groups, and stores this in a new column, weight_group 
# It also groups the variable VehicleAge into four groups
###### This is only an example. You need to create your own groups,
###### with breaks that suit your data
...
glmdata$weight_group <- cut(glmdata$Weight, 
                       ...
                       labels = c("01_<1000kg", "02_1000-1999kg",
                                  "03_2000-3999kg",
                                  "04_>=4000kg"), 
                       right = FALSE)
...
glmdata$age_group <- cut(glmdata$VehicleAge, 
                            ...
                            labels = c("01_<5years",
                                       "02_6-14years", "03_>14years"),
                            right = FALSE)



glmdata$ActivityCode <- gsub("A - Agriculture, Hunting and Forestry", "Heavy duty",
                             glmdata$ActivityCode)
glmdata$ActivityCode <- gsub("C - Mining and quarrying", "Heavy duty",
                             glmdata$ActivityCode)
glmdata$ActivityCode <- gsub("F - Construction", "Heavy duty",
                             glmdata$ActivityCode)


glmdata$ActivityCode <- gsub("G - Wholesale & retail trade; repair of motor vehicles, household", "Regular duty",
                             glmdata$ActivityCode)
glmdata$ActivityCode <- gsub("H - Hotels and restaurants", "Regular duty",
                             glmdata$ActivityCode)
glmdata$ActivityCode <- gsub("I - Transport, storage and communication", "Regular duty",
                             glmdata$ActivityCode)
glmdata$ActivityCode <- gsub("L - Public administration and defence; compulsory social security", "Regular duty",
                             glmdata$ActivityCode)
glmdata$ActivityCode <- gsub("M - Education", "Regular duty",
                             glmdata$ActivityCode)
glmdata$ActivityCode <- gsub("N - Health and social work", "Regular duty",
                             glmdata$ActivityCode)
glmdata$ActivityCode <- gsub("Other", "Regular duty",
                             glmdata$ActivityCode)
...
# That is, instead of having one row per tractor, we want one row for each existing
# combination of variables 
# This code aggregates columns 6-8 of glmdata, by three variables: weight_group, Climate,
# and ActivityCode 
# The aggregated data is stored in a new table, glmdata2 
##### You need to consider if there are any other variables you want to aggregate by,
...
# This piece of code creates a new table, glmdata3, with a row per variable and group,
# and with data on the total duration corresponding to this group.
...
# The first part of this performs a GLM analysis,
# with glmdata2 as the data source modelling NoOfClaims, by the Duration.
...
# in a column named rels.frequency
...
# In other words, the code takes the GLM coefficients for the six weight groups
